...
public class Clients_page_p {
    ...
    public static void main(String[] args) {
        ...
        JFrame frame = new JFrame("EmployInverse - All Clients");
        ...
        JLabel subtitleLabel = new JLabel(clients.size() + " client" + (clients.size() != 1 ? "s" : "") + " found");
        ...
        // ---------- Clients Content ----------
        JPanel clientsContent;

        if (clients.isEmpty()) {
            // Empty state
            clientsContent = createEmptyState();
        } else {
            // Applicants list
            clientsContent = createClientsList(frame);
        }

        contentPanel.add(titlePanel, BorderLayout.NORTH);
        contentPanel.add(clientsContent, BorderLayout.CENTER);
    ...
    private static JPanel createEmptyState() {
        ...
        // Message
        JLabel messageLabel = new JLabel("No clients yet");
        messageLabel.setFont(new Font("Segoe UI", Font.BOLD, 24));
        messageLabel.setForeground(new Color(100, 116, 139));
        messageLabel.setAlignmentX(Component.CENTER_ALIGNMENT);

        // Subtitle
        JLabel subtitleLabel = new JLabel("When clients register in this site, they will appear here.");
        subtitleLabel.setFont(new Font("Segoe UI", Font.PLAIN, 14));
    ...
    private static JPanel createClientsList(JFrame frame) {
        ...
        clientsListPanel.setBackground(new Color(245, 247, 250));

        for (int i = 0; i < clients.size(); i++) {
            Client client = clients.get(i);
            JPanel clientCard = createClientCard(client, i + 1, frame);
            clientsListPanel.add(clientCard);
            clientsListPanel.add(Box.createRigidArea(new Dimension(0, 15)));
        }
    ...
    private static JPanel createClientCard(Client client, int number, JFrame parentFrame) {
        ...
        JLabel usernameLabel = new JLabel("@" + client.getUser_name());
