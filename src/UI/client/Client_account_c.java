...
//        client = me_client;
        ...
        JLabel titleLabel = new JLabel(isMe ? "My Account" : client.getName() + "'s Profile");
        titleLabel.setFont(new Font("Segoe UI", Font.BOLD, 28));
        titleLabel.setForeground(new Color(30, 41, 59));

        JLabel subtitleLabel = new JLabel(isMe ? "Manage your account information" : "Client profile details");
        ...
        // Create Job Button (only if viewing own account)
        if (isMe) {
            gbc.gridx = 0;
            gbc.gridy = 1;
            gbc.gridwidth = 2;
            ...
            gridPanel.add(createJobButton(frame), gbc);
        }
    ...
    private static JPanel createAccountInfoCard() {
        ...
        // Only the own account can see his balance
        if (isMe) {
            card.add(createDivider());
            card.add(createInfoRow("Balance", "$" + client.getBalance()));
        }
    ...
    private static JPanel createJobButton(JFrame parentFrame) {
        JPanel buttonPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
        buttonPanel.setBackground(new Color(245, 247, 250));

        JButton createJobBtn = new JButton("+ Create New Job Post");
        createJobBtn.setFont(new Font("Segoe UI", Font.BOLD, 15));
        createJobBtn.setForeground(Color.WHITE);
        createJobBtn.setBackground(new Color(16, 185, 129));
        createJobBtn.setBorder(BorderFactory.createEmptyBorder(14, 30, 14, 30));
        createJobBtn.setFocusPainted(false);
        createJobBtn.setCursor(new Cursor(Cursor.HAND_CURSOR));

        createJobBtn.addMouseListener(new java.awt.event.MouseAdapter() {
            public void mouseEntered(java.awt.event.MouseEvent evt) {
                createJobBtn.setBackground(new Color(5, 150, 105));
            }
            public void mouseExited(java.awt.event.MouseEvent evt) {
                createJobBtn.setBackground(new Color(16, 185, 129));
            }
